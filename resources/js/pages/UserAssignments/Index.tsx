...
export default function Index({ users, departments, designations }: any) {
    ...
    const filteredUsers = users.filter((user: any) => {
        const matchesId = user.employee_id.toLowerCase().includes(searchId.toLowerCase());
        const matchesName = user.name.toLowerCase().includes(searchName.toLowerCase());
        const matchesDesignation = searchDesignation === '' ||
            user.assignment?.designation?.id?.toString() === searchDesignation;
        ...
        <AppLayout>
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    />
                    <select
                        className="border rounded px-3 py-2 w-full focus:outline-none focus:ring focus:ring-purple-300"
                        value={searchDesignation}
                        onChange={(e) => setSearchDesignation(e.target.value)}
                    >
                        <option value="">Filter by Designation (All)</option>
